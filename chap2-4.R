...
## ---- plot-return ----
indexes = seq(100)
df = build_df(timesteps = indexes, data=X.return, headers=c('date', 'X'), rowindexes=NULL)
df = merge_df(df=df, data=Y.return, col_name='Y')
...
## ---- plot-price ----
indexes2 = seq(101)
df2 = build_df(timesteps = indexes2, data=X.price, headers=c('date', 'X'), rowindexes=NULL)
df2 = merge_df(df=df2, data=Y.price, col_name='Y')
...
df_stat = build_df(mat_X, mat_Y, mat_headers, mat_rownames)
print(df_stat)
